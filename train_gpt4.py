...
from models.vqgan import VQGAN4 as VQGAN
from models.clip import clip_vit_b32
from models.gpt2 import __dict__ as gpts
...
parser.add_argument("--vqgan_ckpt", type=str, default='/data/yzh/VQGAN-CLIP/taming-transformers/logs/2025-04-05T14-36-19_custom_vqgan_medical/lightning_logs/2025-04-05T14-36-19_custom_vqgan_medical/checkpoints/epoch=32-step=627000.ckpt',help="VQGAN pretrained model")
...
def train(dataloader, gpt, vqgan, clip, optimizer, scheduler, scaler, epoch, start_step, best_score):
    ...
    for step, batch in enumerate(dataloader):
        ...
        # 获取成对数据（已合并为[B*2, ...]）
        x, target_images = batch['vqgan_imgs'].cuda().chunk(2, dim=0) # [B*2, C, H, W]
        clip_x, _ = batch['clip_imgs'].cuda().chunk(2, dim=0)
        # print("vqgan_imgs: ", vqgan_imgs)

        # 标签处理 --------------------------------------------------
        # 目标图像（ground truth）始终取第二张配对图像
        print('train_gpt4.py:173  x: ', x.shape)

        print("train_gpt4.py:175 clip_x: ", clip_x.shape)
        print("train_gpt4.py:176 target_images.shape: ", target_images.shape)

        print(f"Starting epoch {epoch}, batch size: {batch_size}")
        print(f"First batch shapes - x: {x.shape}, clip_x: {clip_x.shape}")
        ...
        with torch.no_grad():
            ...
            '''
            # 直接让x = clip_x，因为clip_x好像是x的语义映射。
            embeddings = clip.encode_image(clip_x)  # [B, 512]
            # print("train_gpt4.py:200 embeddings.shape203: ",embeddings.shape)
        L = indices.size(1)
        # print("train_gpt4.py:202 L: ", L)
        ...
        with torch.amp.autocast('cuda',enabled=use_amp):
            # print('train_gpt4.py:210 embeddings.shape',embeddings.shape)
        ...
        print(f"train_gpt4.py:259 GPU memory allocated: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
        # log
...
def custom_collate(batch):
    ...
    # 转换为张量
    return {
        'vqgan_imgs': torch.stack(paired_batch['vqgan_imgs']),
...
def main():
    ...
    train_dataset = PairedDataset(
        root_dir="data/spine2/images/train",
        annotation_csv="data/annotations2/train.csv",
        transform=KeypointTransform(split='train')
    )

    val_dataset = PairedDataset(
        root_dir="data/spine2/images/val",
        annotation_csv="data/annotations2/val.csv",
    ...
    print("Optimizer state:", optimizer.state_dict().keys())
